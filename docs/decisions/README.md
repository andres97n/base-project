...
- **[0003-typescript-6-upgrade.md](./0003-typescript-6-upgrade.md)** — upgrading to TypeScript 6.0: replacing deprecated `baseUrl` with `paths`, pinning `rootDir`/`types` against new 6.0 defaults, and deliberately keeping `strict: false`.
- **[0004-maintenance-and-release-policy.md](./0004-maintenance-and-release-policy.md)** — adopting semver + release governance (v1.0.0 baseline), pnpm-translated dependency/deprecation policy, CI quality gate, Renovate, and the clean-architecture layer mapping (no restructure).
